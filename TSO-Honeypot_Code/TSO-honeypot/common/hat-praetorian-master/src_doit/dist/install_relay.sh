...
unlink $BIN/hat-praetorian-relay
rm $SYSTEM/hat-praetorian-relay.service
...
mkdir -p $DATA_PATH
[ ! -f $DATA_PATH/relay.yaml ] && cat > $DATA_PATH/relay.yaml < relay.yaml
chmod 0666 $DATA_PATH/relay.yaml

ln -sf $INSTALL_PATH/bin/hat-praetorian-relay $BIN/hat-praetorian-relay

mkdir -p $SYSTEM
cat > $SYSTEM/hat-praetorian-relay.service << EOF
[Unit]
Description=Hat PRAETORIAN relay
After=network.target
Requires=network.target

[Service]
WorkingDirectory=$DATA_PATH
ExecStart=$BIN/hat-praetorian-relay
